...
import { FormType, SysDefine } from "./SysDefine";
import { MaskType } from "./FrameType";
import Binder from "./Binder";
import AdapterMgr from "../Manager/AdapterMgr";
...
export default class UIBase extends UIBinder {
    ...
    //按钮点击监听
    public onClickAdd(button:cc.Button,func:Function,uType?:UIBase){
        if(uType == null)
            button.node.on("click",func)
        else
            button.node.on("click",func,uType)
    }
}
